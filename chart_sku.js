...
async function Process_data_sku_per_day(dateSince , dateTo , days_line_y , type_of_date , brand){
    ...
    //get json
      ...
      for ( var i =0; i<result.length; i++){
         ...
         array_of_company_revenue_sixth[i] = result[i].revenue;
         console.log(days_line_y[i]);
         console.log(array_of_company_revenue_sixth[i]);
      }
  ...
  var d_to = dateTo;
    ...
    //get json
      ...
      for ( var i =0; i<result.length; i++){
        ...
        console.log("Results:" , array_of_company_revenue_sixth)
...
current = result[0].week;
  ...
  for (var i = 0; i < result.length ; i++){
    ...
    if(current == result[i].week ){
      array_of_company_revenue_sixth[position] +=result[i].revenue;
